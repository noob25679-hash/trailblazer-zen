...
export default function MapScreen() {
  const { trails, userLatLng, loadTrailsForArea, isLoadingTrails, savedTrails, toggleSave, showToast } = useApp();
  ...
  const initMap = useCallback(() => {
    ...
    // Auto-load trails on map move
    map.on('moveend', () => {
      clearTimeout(debounceRef.current);
      debounceRef.current = setTimeout(() => {
        const center = map.getCenter();
        const zoom = map.getZoom();
        loadTrailsForArea(center.lat, center.lng, zoom);
      }, 1000);
    });
  ...
  }, [userLatLng, loadTrailsForArea]);

  const addUserDot = (map: L.Map, ll: [number, number]) => {
  ...
  // Init map when screen becomes active
  useEffect(() => {
    setTimeout(initMap, 100);
  }, [initMap]);

  // Render markers when trails change - limit visible markers to prevent congestion
  useEffect(() => {
  ...
  const locateMe = () => {
    ...
    navigator.geolocation.getCurrentPosition(pos => {
      ...
      showToast('📍 Found you!');
    }, () => showToast('⚠️ Could not get location'), { timeout: 10000 });
  };
  ...
  return (
    <div className="absolute inset-0" style={{ bottom: 'calc(72px + env(safe-area-inset-bottom, 0px))' }}>
      ...
      {/* HUD */}
      <div className="absolute top-[52px] left-4 right-4 flex justify-between items-center pointer-events-none z-[500]">
        <div className="pointer-events-auto bg-background/90 backdrop-blur-xl border border-border-bright rounded-full px-4 py-2 flex items-center gap-2">
          <div className={`w-1.5 h-1.5 rounded-full transition-colors ${isLoadingTrails ? 'bg-primary animate-pulse-green' : 'bg-text-dim'}`} />
          <span className="font-mono text-[10px] text-secondary-foreground tracking-[1px]">
            {markersRef.current.length} TRAILS
          </span>
        </div>
        <div onClick={locateMe} className="pointer-events-auto cursor-pointer bg-background/90 backdrop-blur-xl border border-border-bright rounded-full px-4 py-2 flex items-center gap-2">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="2"><circle cx="12" cy="12" r="3"/><path d="M12 1v4M12 19v4M1 12h4M19 12h4"/></svg>
          <span className="font-mono text-[10px] text-secondary-foreground tracking-[1px]">LOCATE</span>
        </div>
      </div>

      {/* Map controls */}
      <div className="absolute right-4 top-[110px] z-[500] flex flex-col gap-2">
        <button onClick={() => mapRef.current?.zoomIn()} className="w-[42px] h-[42px] rounded-full bg-background/90 border border-border-bright flex items-center justify-center cursor-pointer text-secondary-foreground text-[20px] font-light">+</button>
        <button onClick={() => mapRef.current?.zoomOut()} className="w-[42px] h-[42px] rounded-full bg-background/90 border border-border-bright flex items-center justify-center cursor-pointer text-secondary-foreground text-[20px] font-light">−</button>
      </div>
      ...
      {sheetTrail && (
        <div className="absolute bottom-20 left-3 right-3 z-[600] bg-card border border-border-bright rounded-[20px] p-3.5 flex items-center gap-3 animate-slide-up">
          ...
          <div className="flex-1 min-w-0">
            <div className="font-bold text-[14px] text-foreground whitespace-nowrap overflow-hidden text-ellipsis">{sheetTrail.title}</div>
          ...
          <button onClick={() => setSheetTrail(null)} className="bg-transparent border-none text-text-dim text-[20px] cursor-pointer p-1">×</button>
        </div>
      )}
    </div>
  );
